...
func TestGetGopherTool(t *testing.T) {
	runTest := func(
		tb testing.TB,
	) (
	...
	t.Run("can get existing gopher", func(t *testing.T) {
		...
		assert.True(t, ok)
		assert.Equal(t, `{"id":2,"name":"bob","job":"backend"}`, resContents.Text)
		...
		tracetest.AssertHasTraceSpan(
			t,
			traceExporter,
			"MCP tools/call get-gopher",
			attribute.String("mcp.method", "tools/call"),
			attribute.String("mcp.tool", "get-gopher"),
        ...
        mcp_server_requests_total{method="tools/call",status="success",target="get-gopher"} 1
    `
	...
	t.Run("cannot get invalid gopher", func(t *testing.T) {
		testServer, logBuffer, traceExporter, metricsRegistry := runTest(t)
        ...
        # TYPE mcp_server_requests_total counter
		mcp_server_requests_total{method="initialize",status="success",target=""} 1
        mcp_server_requests_total{method="tools/call",status="error",target="get-gopher"} 1
    `
